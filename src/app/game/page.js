...
function randomPiece() {
  ...
  return {
    shape: p.shape,
    color: p.color,
    x: Math.floor(BOARD_W / 2) - Math.floor(p.shape[0].length / 2),
    y: 0,
  };
}
...
function TetrisGame() {
  ...
  const handleSubmitScore = async (e) => {
    ...
    try {
      const localScores = JSON.parse(
        localStorage.getItem("game_scores_local") || "[]",
      );
  ...
  const drawBoard = useCallback(() => {
    ...
    if (st.current && !st.gameOver) {
      ...
      while (isValid(st.board, ghost, 0, 1))
  ...
  const moveLeft = useCallback(() => {
    const st = stateRef.current;
    if (!st.running || st.gameOver) return;
    if (isValid(st.board, st.current, -1)) {
      st.current = { ...st.current, x: st.current.x - 1 };
      drawBoard();
    }
  }, [drawBoard]);
  ...
  const hardDrop = useCallback(() => {
    const st = stateRef.current;
    if (!st.running || st.gameOver) return;
    while (isValid(st.board, st.current, 0, 1)) {
      st.current = { ...st.current, y: st.current.y + 1 };
    }
    drawBoard();
  }, [drawBoard]);
  ...
  return (
    <div className="game-wrap">
      <div className="game-left" style={{ position: "relative" }}>
        ...
        {showSubmitModal && (
          <div
            className="leaderboard-submit-overlay"
            style={{
              position: "absolute",
              top: 0,
              left: 0,
          ...
          >
            ...
            >
              Kamu meraih skor <strong>{display.score}</strong>. Masukkan nama
              kamu untuk Papan Peringkat:
            </p>
            {!scoreSubmitted ? (
              <form
      ...
      <div className="mobile-controls">
        <div className="ctrl-row">
          <button className="ctrl-btn" onClick={rotate}>
            ↻ Rotate
          </button>
        </div>
        <div className="ctrl-row">
          <button className="ctrl-btn" onClick={moveLeft}>
            ◀
          </button>
          <button className="ctrl-btn" onClick={moveDown}>
            ▼
          </button>
          <button className="ctrl-btn" onClick={moveRight}>
            ▶
          </button>
        </div>
        <div className="ctrl-row">
          <button className="ctrl-btn ctrl-btn-wide" onClick={hardDrop}>
            ⬇ DROP
          </button>
        </div>
      </div>
...
function GameMenu({ onSelectGame }) {
  const games = [
    ...
    },
    {
      ...
      name: "Tic Tac Toe",
      icon: "⭕",
      desc: "Mainkan X dan O sampai menang!",
      color: "#ffd700",
...
export default function GamePage() {
  ...
  const fetchScores = useCallback(async (gameId) => {
    ...
    try {
      ...
      console.warn(
        "Koneksi Supabase error / table belum dimigrasi, fallback ke lokal:",
        err,
      );
    }
      ...
      try {
        ...
        if (settingsData) {
          settingsData.forEach((row) => {
            if (row.key === "theme_color" && row.value) {
              setThemeColor(row.value);
              localStorage.setItem("theme_color", row.value);
            }
            if (row.key === "bg_theme" && row.value) {
              setBgTheme(row.value);
              localStorage.setItem("bg_theme", row.value);
            }
            if (row.key === "font_choice" && row.value) {
              setFontChoice(row.value);
              localStorage.setItem("font_choice", row.value);
            }
            if (row.key === "default_theme" && row.value) {
              setIsDark(row.value === "dark");
              localStorage.setItem("default_theme", row.value);
            }
            if (row.key === "bg_animation" && row.value) {
              setBgAnimation(row.value);
              localStorage.setItem("bg_animation", row.value);
            }
          });
        }
      } catch (e) {
  ...
  const gameTitles = {
    menu: {
      title: "Game Arcade",
      sub: "Pilih game favoritmu dan mulai bermain!",
    },
    tetris: { title: "🧩 Tetris", sub: "Susun blok dan raih skor tinggi!" },
    snake: {
      title: "🐍 Snake",
      sub: "Makan makanan dan jangan tabrak dinding!",
    },
    memory: { title: "� Memory", sub: "Temukan pasangan kartu yang cocok!" },
    tictactoe: {
      title: "⭕ Tic Tac Toe",
      sub: "Mainkan X dan O sampai menang!",
    },
    flappybird: { title: "🐦 Flappy Bird", sub: "Terbang dan hindari pipa!" },
  };
    ...
    <>
      ...
      <div className="game-page">
        <nav className="game-nav">
          <Link href="/" className="game-nav-logo">
            aura<em>a</em>uvarose
          </Link>
          ...
          {selectedGame === "tetris" ? (
            <>
              ...
              <div className="game-tips">
                <strong>Desktop:</strong> ← → bergerak &nbsp;·&nbsp; ↑ rotate
                &nbsp;·&nbsp; ↓ turun &nbsp;·&nbsp; Space drop
                <br />
                <strong>Mobile:</strong> Swipe kiri/kanan bergerak &nbsp;·&nbsp;
                Swipe atas rotate &nbsp;·&nbsp; Swipe bawah drop &nbsp;·&nbsp;
                Tap rotate
              </div>
            </>
          ) : (
              ...
              {selectedGame === "menu" && (
                <div className="arcade-leaderboard">
                  <h2 className="leaderboard-title">
                    🏆 PAPAN PERINGKAT ARCADE
                  </h2>
                  <div className="leaderboard-tabs">
                    <button
                      className={`leaderboard-tab ${leaderboardTab === "tetris" ? "active" : ""}`}
                      onClick={() => setLeaderboardTab("tetris")}
                    >
                      🧩 Tetris
                    </button>
                    <button
                      className={`leaderboard-tab ${leaderboardTab === "snake" ? "active" : ""}`}
                      onClick={() => setLeaderboardTab("snake")}
                    >
                      🐍 Snake
                    </button>
                    <button
                      className={`leaderboard-tab ${leaderboardTab === "memory" ? "active" : ""}`}
